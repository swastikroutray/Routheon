...
def init_providers() -> dict:
    ...
    if config.GEMINI_API_KEY:
        ...
        logger.warning(
            "⚠ GEMINI_API_KEY not set — Gemini provider unavailable"
        )
    ...
    if config.GROQ_API_KEY:
        ...
        logger.warning(
            "⚠ GROQ_API_KEY not set — Groq provider unavailable"
        )
...
async def route_prompt(prompt: str) -> dict:
    ...
    logger.info(
        f"[{request_id[:8]}] Classified as tier {tier}: {signals}"
    )
    ...
    if provider is None:
        ...
        for fallback_name in [
            ...
            if fallback_name in providers:
                ...
                # Default model for fallback provider
    ...
    try:
        ...
        error_detail = (
            ...
            if str(e)
                ...
                f"{type(e).__name__} "
                "(no message — likely a timeout)"
            )
        )

        logger.error(
            f"[{request_id[:8]}] "
            f"Provider error: {error_detail}",
            exc_info=True,
        )
    ...
    try:
        ...
        logger.error(
            f"[{request_id[:8]}] DB logging error: {e}"
        )
...
app = FastAPI(
    title="LLM Router",
    description=(
        "Zero-cost multi-provider LLM routing proxy "
        "with Gemini-compatible API"
    ),
    version="1.0.0",
    lifespan=lifespan,
)
...
async def health():
    ...
    return {
        ...
        "tiers_configured": list(
            routing_config.get("tiers", {}).keys()
        ),
    }
...
if frontend_dist.exists():
    ...
    async def serve_frontend(
        ...
        If the requested path corresponds to an actual
        file, return that file.

        Otherwise return index.html so React can handle
        client-side routing.
        """
    ...
    logger.warning(
        "React frontend build not found at %s. "
